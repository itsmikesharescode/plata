		...
		{#snippet child({ props })}
			<div class="flex w-full justify-end">
				<Button {...props} variant="ghost" class="flex h-8 w-8 p-0 data-[state=open]:bg-muted">
					<Ellipsis />
					<span class="sr-only">Open Chairperson Action</span>
				</Button>
			</div>
		{/snippet}
	</DropdownMenu.Trigger>
	<DropdownMenu.Content class="w-fit" align="end">
		<DropdownMenu.Item
			onclick={async () => {
				...
				await goto(urlParamStacker('id', row.original.user_id, page));
			}}>Edit</DropdownMenu.Item
		>
		<DropdownMenu.Item
			onclick={async () => {
				await goto(urlParamStacker('deletion_id', row.original.user_id, page));
